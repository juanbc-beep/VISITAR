...
| `casos/mfa.mjs` | Verificación en dos pasos (TOTP) del administrador general: alta propia desde «Tu cuenta» (rechaza un código incorrecto antes de aceptar el bueno), que a partir de ahí el login pide el código (ídem, rechaza antes de dejar pasar) y que desactivarla vuelve a dejar entrar sólo con contraseña. El simulador no hace TOTP de verdad — acepta un único código fijo como «correcto» (ver `MFA_CODE_OK` en `simulador.mjs`). |
