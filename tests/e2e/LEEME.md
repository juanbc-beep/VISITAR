...
| `casos/interprete_grupos.mjs` | Los candidatos del Intérprete se agrupan por equivalencia entre nomencladores (EQGRUPO): un solo bloque por práctica, un solo % (antes: dos porcentajes distintos para "el mismo código", 340301 del PMO y del Único), un chip "también en" para elegir el otro nomenclador aparte, y la exposición adicional (340302) como nota con sus propios chips en vez de una tarjeta más. Cubre también el caso real que expuso que exposicion_siguiente sólo está cargado del lado del PMO: si el principal del grupo termina siendo el del Único, la relación se buscaba en el grupo entero, no sólo en el principal. Caso real de una captura del usuario. Ver HANDOFF.md, 26/8/2026. |
...
| `casos/contrataciones.mjs` | La grilla del prestador sale como grilla de carga del Único (nom_nom, rango, área, 4 importes, 4 unidades) más la hoja «Sin equivalencia»: títulos arriba del encabezado, rangos, lista abreviada resuelta por la práctica, NN/NBU por unidad, código repetido con elección de valor, corrección a mano; se lee el `.xls` descargado y se compara fila por fila. Un usuario no admin no ve el botón. Ver HANDOFF.md 4.9 bis. |
| `casos/pistas.mjs` | La pista de la campanita («Acá te avisamos qué cambió») no vuelve al siguiente scroll después de «Después» o Escape (sí al volver a abrir la app), «Entendido» la cierra para siempre, y no aparece encima de una ventana abierta ni por scrollear adentro de ella. Reporte del usuario, ver HANDOFF.md 23/9/2026. |
| `casos/legales.mjs` | Las tres páginas legales cargan sin scripts ni pedidos a otro sitio, se enlazan entre sí y traen la leyenda obligatoria de la AAIP; crear una cuenta exige aceptar Términos + Privacidad y guarda versión y fecha; una cuenta con otra versión la acepta una vez al entrar (y «Cerrar sesión» no deja pasar); si ya la aceptó en otro dispositivo no se le vuelve a pedir; del Intérprete de orden no sale nada que identifique al paciente. Ver HANDOFF.md 4.10. |
| `casos/accesibilidad.mjs` | axe-core (WCAG 2.0/2.1/2.2 A y AA) sobre todas las pantallas principales, el acceso, la aceptación de términos, Contrataciones, cada pestaña de Administración y las páginas legales, en tema claro y oscuro. Cualquier violación hace fallar. Espera a que terminen las animaciones antes de medir (si no, mide colores a mitad de un fundido). Ver HANDOFF.md 4.10. |
