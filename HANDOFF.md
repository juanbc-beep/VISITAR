...
|---|---|
| **Activar la verificación en dos pasos quedaba trabada** con «A factor with the friendly name "Administrador" for this user already exists», sin forma de reintentar sin entrar al panel de Supabase (pasó en producción el 25/8/2026, sobre el propio administrador) | El primer intento no mostró el QR (motivo no confirmado — no hay forma de reproducir el llamado real a Supabase desde este entorno) y dejó un factor **sin verificar** cargado del lado del servidor; Supabase no deja crear otro con el mismo nombre mientras exista uno, verificado o no. `NUBE.enrolarTOTP()` ahora **borra los factores TOTP sin verificar de la cuenta antes de pedir uno nuevo** — sólo los que nunca se verificaron, nunca toca el que está activo de verdad. De paso, la pantalla de alta quedó más robusta: si la nube no manda ni QR ni secreto avisa en vez de dejar tipear un código contra nada, y si el `<img>` del QR falla al cargar (por CSP no puede ser un `onerror="…"` inline: se engancha con `addEventListener`) el secreto manual sigue visible y usable igual |
