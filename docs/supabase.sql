...
--
--  La regla de los pacientes no la puede hacer cumplir la base: «perfiles.notas»
--  es texto libre y nadie puede impedir que alguien escriba un apellido ahí. Por
--  eso el aviso está también en la app, al lado del campo. Si alguna vez hace
--  falta comprobarlo, la consulta es sobre esa columna.
--
...
-- ---------------------------------------------------------------------
-- 10. RASTRO DE AUDITORÍA — quién cambió qué, y cuándo
--
--    La app anota las ediciones en «CONTENT.log», pero eso vive en el
--    navegador de quien editó y se corta a 200 entradas: es una nota
--    personal, no un registro. Y las bajas no dejaban rastro en ningún lado.
--
...
--
--    Si tu base ya está andando, esto mismo está en supabase_auditoria.sql.
-- ---------------------------------------------------------------------
